...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        alien.addImage("z.jpg","a picture of ET");
        alien.addImage("z.jpg","a picture of ET 2"); // this will overwrite the previous entry, because the key is the same
        alien.addImage("a.jpg", "ET in the air on a bike");

        long generatedId = (Long)session.save(alien);
        tx.commit();

        Alien readFromDB = session.get(Alien.class,generatedId);
        readFromDB.getImages();
